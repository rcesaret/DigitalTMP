...
pip install papermill nbformat markdown-link-check
# set up pre-commit hooks
pre-commit install
```

Ensure Graphviz is installed for ERD generation. For more details, see `PLANNING_PHASE1.md#3.1` and `architecture.md#deployment`.

### 2.2 The Four Workflows of Phase 1
...
4. **Workflow 4: Analysis, Drafting & Reporting**: This workflow is subdivided into: (1) execution of six individual analysis notebooks (Task 4.3); (2) drafting of interpretive prose within each executed notebook (Task 4.4); (3) execution of the comparative analysis notebook (Task 4.3 revisited); (4) drafting of the comparative narrative (Task 4.5); and (5) automated assembly of all drafted artefacts into the Phase 1 white paper (Task 4.6).
...
├── reports/                                        # Jupyter notebooks for analysis
│   ├── individual_db_analysis/                     # Individual database analysis notebooks
│   │   ├── executed/                               # Code-only executed notebooks
│   │   │   ├── report_TMP_DF8.ipynb
│   │   │   ├── report_TMP_DF9.ipynb
│   │   │   ├── report_TMP_DF10.ipynb
│   │   │   ├── report_tmp_rean_df2.ipynb
│   │   │   ├── report_tmp_benchmark_wide_numeric.ipynb
│   │   │   └── report_tmp_benchmark_wide_text_nulls.ipynb
│   │   └── drafted/                                # Notebooks with completed narratives
│   │       ├── report_TMP_DF8.drafted.ipynb
│   │       ├── report_TMP_DF9.drafted.ipynb
│   │       ├── report_TMP_DF10.drafted.ipynb
│   │       ├── report_tmp_rean_df2.drafted.ipynb
│   │       ├── report_tmp_benchmark_wide_numeric.drafted.ipynb
│   │       └── report_tmp_benchmark_wide_text_nulls.drafted.ipynb
│   └── comparative_db_analysis/                    # Comparative analysis across all databases
│       ├── executed/
│       │   └── 06_Comparative_Analysis_Report.ipynb
│       └── drafted/
│           └── 06_Comparative_Analysis_Report.drafted.ipynb
├── notebooks/                                      # Notebook templates
│   ├── template_individual_db_analysis.ipynb      # Template for individual DB analysis
│   └── template_comparative_analysis.ipynb        # Template for comparative analysis
├── scripts/                                        # Utility scripts
│   └── validate_cell_tags.py                       # Script to validate cell tags in notebooks
├── configs/                                        # Configuration files
│   └── cell_tag_schema.json                        # Schema defining required notebook cell tags
├── drafts/                                         # Contains working drafts of narrative reports.
│   └── Phase1_WhitePaper_v3.md                    # The final deliverable white paper for this phase.
├── tests/                                          # Test suites
└── README.md                                       # This file. The main documentation for Phase 1.
```

*Note: Executed notebooks contain code outputs only; drafted notebooks include narrative Markdown sections. The comparative notebook tracks cross-database metrics synthesis and narrative drafting.*
...
* **Version Control**: Git
* **QA & Validation**: papermill, markdown-link-check, nbformat
...
* **`src/04_run_comparison.py`**:
    * **Objective**: To aggregate all raw metrics into final summary reports.
    * **Description**: Reads all CSV/JSON files from `outputs/metrics/`, aggregates and synthesizes the data, and produces two final reports: a machine-readable CSV and a human-readable Markdown report.
    * **Inputs**: All raw metric files from Workflow 2.
    * **Outputs**: `comparison_matrix.csv` and `comparison_report.md` in `outputs/reports/`.

### 3.4 Workflow 4: Assets for Analysis, Drafting & Reporting

**Task Mapping to TASKS.md:**
1. **Execute** individual notebooks — [P1.W4.T3.1–P1.W4.T3.4](TASKS.md#P1.W4.T3.1)
2. **Draft** individual narratives — [P1.W4.T3.1–P1.W4.T3.4](TASKS.md#P1.W4.T3.1)
3. **Execute** comparative notebook — [P1.W4.T3.5](TASKS.md#P1.W4.T3.5)
4. **Draft** comparative narrative — [P1.W4.T3.5](TASKS.md#P1.W4.T3.5)
5. **QA** narrative drafts — [P1.W4.T3.6](TASKS.md#P1.W4.T3.6)
6. **Assemble** white paper — [Task 4.6](TASKS.md#P1.W4.T2.3)

* **`notebooks/template_individual_db_analysis.ipynb`**: A reusable template for conducting a deep-dive analysis of a single database. It is run once for each of the six databases to produce a standardized report. The provided notebook templates establish the structural scaffolding for each analysis workflow, including pre-configured data ingestion and profiling cells. Subsequent to execution, analysts must complete narrative sections—Introduction, Methods, Results, Discussion, and Recommendations—within designated Markdown cells (tagged per the [Cell-Tag Schema](configs/cell_tag_schema.json)). This drafting responsibility is formalized in Tasks P1.W4.T3.1–P1.W4.T3.6.
...
* **`drafts/Phase1_WhitePaper_v3.md`**: The initial draft of the final narrative report for this phase. The outputs of Workflow 4 are used to revise and complete this document.

* **`scripts/validate_cell_tags.py`**: Defines the cell-tag schema and must be run on drafted notebooks prior to submission.
...
The execution of this phase produces a rich set of data files, reports, and analytical notebooks. The total number of reported artefacts across Phase 1 is now approximately **48**: 18 executed notebooks (six individual DBs executed twice across runtime regression), 18 drafted notebooks, nine ERDs and three aggregated summary tables. The counts may vary slightly depending on the number of analysis templates.

### 4.1 Notebook Templates (`notebooks/`)

Templates generate *scaffolding*; drafting tasks populate scaffolds with interpretive prose.
...
4. **Execute Workflow 4: Analysis, Drafting & Reporting**
   * Execute the individual analysis notebooks using the templates in `notebooks/`
   * Draft narrative content in each executed notebook following the Cell-Tag Schema
   * Execute the comparative analysis notebook
   * Draft the comparative narrative
   * Run QA validation on all drafted notebooks
   * Synthesize findings into the final white paper

### 7.1 Validation Checkpoints

After each workflow, verify outputs:
- **Workflow 1**: Six databases accessible via psql
- **Workflow 2**: ~48 metric files in `outputs/metrics/`
- **Workflow 3**: Summary reports in `outputs/reports/`
- **Workflow 4**: Drafted notebooks pass cell-tag validation

---

## 8. Appendices

### Appendix A: Notebook Quality Assurance Badge

A new badge has been added to the top of the README to visualise the status of narrative QA in the drafted notebooks. The badge is generated by the continuous integration workflow that runs `validate_cell_tags.py` and `markdown-link-check` on all drafted notebooks. When all notebooks satisfy the cell-tag schema and link checks, the badge appears green. If any narrative cell is missing a tag or contains a broken link, the badge turns red. This badge helps maintainers quickly assess the quality of narrative content across the project.

### Appendix B: Links to Updated Resources

The comparative analysis notebook link previously pointed to a non-existent `template_comparative_analysis.ipynb`. This link has been replaced with two links: one to the **executed comparative notebook** in `reports/comparative_db_analysis/executed/` and another to the **drafted comparative narrative** in `reports/comparative_db_analysis/drafted/`. Screenshots in the README now reflect the drafted narrative stage; for example, the sample image `images/drafted_individual_notebook.png` shows a narrative notebook with commentary and cell tags.

See `changes.md#Appendix A` and `changes.md#Appendix B` for a complete list of modifications enacted in this update.
